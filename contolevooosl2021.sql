...
CREATE TABLE IF NOT EXISTS `aeroportos` (
  ...
  `uf` CHAR(2) NOT NULL,
  `aeroportoscol` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `passageiros` (
  `idpassageiros` INT NOT NULL AUTO_INCREMENT,
  `idviagens` INT NOT NULL,
  `idpessoas` INT NOT NULL,
  `poltrona` VARCHAR(6) NULL,
  INDEX `fk_viagens_has_pessoas_pessoas1_idx` (`idpessoas` ASC) VISIBLE,
  INDEX `fk_viagens_has_pessoas_viagens1_idx` (`idviagens` ASC) VISIBLE,
  PRIMARY KEY (`idpassageiros`, `idpessoas`, `idviagens`),
  CONSTRAINT `fk_viagens_has_pessoas_viagens1`
    FOREIGN KEY (`idviagens`)
    REFERENCES `viagens` (`idviagens`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_viagens_has_pessoas_pessoas1`
